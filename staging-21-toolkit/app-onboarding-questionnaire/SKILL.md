...
description: 'USE THIS SKILL to design and build a high-converting questionnaire-style onboarding flow for an app — 5-phase workflow (Discovery → Transformation → Blueprint → Content → Implementation), modelled on proven patterns from Mob, Headspace, Noom, Duolingo. Persists progress to memory across sessions.'
user-invocable: true
---
